...
## マスター管理・組織管理
管理者は `/admin/organization` から施設の登録から、その施設に紐づく部署の構成までを単一の画面で直感的に管理できます。
初期状態として以下の4施設と各部署がシードデータから提供されています。
- 本部病院 (部署: 理学療法課, 看護部, 事務局)
- 本部病院介護医療院 (部署: 介護課, 看護部)
- 後光病院 (部署: リハビリテーション科, 薬剤科)
- 玉診療所 (部署: 外来, 事務)

これまでの段階的な管理メニューを廃止し、アコーディオンUIとインライン編集により、不要な画面遷移なく組織構造をメンテナンスできるようにUIをシンプル化しました。
また、システムで利用する職種マスターの管理は `/admin/professions` に独立して配置し、権限に応じた柔軟な拡張性を維持しています。
初期データとして以下の7職種が想定されています：理学療法士、作業療法士、言語聴覚士、看護師、介護職、事務職、その他。
