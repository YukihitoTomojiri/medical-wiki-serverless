...
## マスター管理・組織管理
管理者・開発者は `/admin/masters` から組織の基盤となるマスターデータ（施設、部署、職種）の一覧・追加・編集・削除が一括で行えます。
また、`/admin/organization` からは実際の組織構成（全体像）を確認できます。初期状態として以下の4施設がシードデータから提供されています。
- 本部病院 (部署: 理学療法課, 看護部, 事務局)
- 本部病院介護医療院 (部署: 介護課, 看護部)
- 後光病院 (部署: リハビリテーション科, 薬剤科)
- 玉診療所 (部署: 外来, 事務)

これまでバラバラだった組織管理・職種管理のメニューが統合され、専用のタブUI機能が実装されました。
