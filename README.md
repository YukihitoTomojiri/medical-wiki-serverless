...
- **管理者ダッシュボード（稼働ノード管理）**: 
  ...
  - スマホ表示時専用（`md`未満）に余分な縦余白（パディング）を極限まで削減し、ステータスバッジをユーザー名の横にインライン配置しました。さらに各種アクションボタンをアイコンのみの最小サイズに縮小することで、1画面に収まるデータ量を最大化しました。この際、PC（ブレークポイント `md` 以上）向けのゆとりあるレイアウトは完全に維持されています。
