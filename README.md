...
=======
## 📱 モバイル・タブレット対応 (Responsive Design)
すべての管理画面・メインダッシュボードはモバイルファーストおよびレスポンシブデザインに対応しています。スマートフォンやタブレットからのアクセスでも快適な操作を提供します。

- **ナビゲーションドロワー**: 画面幅が狭い場合、サイドバーはハンバーガーメニューから開閉するドロワー（スライドイン）に変化します。
- **カードレイアウトテーブル**: ユーザー一覧や研修一覧のテーブルは、画面幅に応じて自動的に見やすいカードレイアウトへ変形します。
- **タッチ操作の最適化**: 各種ボタンや入力フォームは、モバイル端末での誤タップを防ぐため十分なタップ領域（min-height: 48px）と余白を確保しています。
- **自動フォントスケーリング**: 環境の画面サイズに応じて、文字サイズ（Typography）が読みやすく自動調整されます。
>>>>>>> origin/main

## 検討事項・要相談事項

### 画像アップロードのインフラ選定（リッチテキストエディタ用）
* **ステータス**: 要相談（実装保留）
* **背景**: 新規投稿画面（マニュアル・お知らせ等）にTiptapエディタを導入し、テキスト装飾までは完了。次に画像アップロード機能を実装するにあたり、画像の保存先を決める必要がある。
* **有力候補**: Cloudflare R2
  * *メリット*: バックエンド（Cloudflare Workers）との親和性が高く、無料枠（10GB/月）でほぼ賄える。通信費も無料。
* **懸念点・相談事項**: R2の利用開始には、Cloudflareアカウントへの「クレジットカード登録」が必須となる。社内ルールや運用方針と照らし合わせ、このままR2を採用して進めるか、別の代替手段を検討するか方針を決定する。
