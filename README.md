...
- **全ページ共通の見出し（ヘッダー）1行化およびデザイン完全統一**:
  - 各ページのタイトル部（`PageHeader` や `AdminPageLayout`）に対し、スマホ環境（`md`未満）限定で上下の余白を極小化し、装飾のサブタイトル（説明文）を非表示（`hidden md:block`）にする「引き算のデザイン」を適用しました。
  - さらに、マスタ管理や運用管理内の各コンポーネント（研修会一覧、組織・施設管理、職種管理、ユーザー管理、お知らせ管理等）に点在していたローカル見出しや独自スタイルを撤廃し、アイコン＋タイトルの統一規格デザイン（`text-base md:text-lg`）に完全統合しました。
  - これにより、PC表示（`md`以上）ではプロフェッショナルな統一感を確立し、スマホ表示では画面を開いた瞬間にタイトルとアクションボタンがスッキリと「横1行」に収まり、メインコンテンツへ素早くアクセスできるようスクロール量を削減しています。
